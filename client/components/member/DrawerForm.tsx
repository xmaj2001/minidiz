...
import {
  FormDataRegisterMember,
  SchemaRegisterMember,
} from "@/http/member/member.schema";
...
/*
export function MemberRegistrationForm({ onCreating }: FormProps) {
...
interface FormProps{
  onCreate: (member: IMember | null) => void;
}
export function DrawerFormRegisterMember({onCreate}: FormProps) {
  const [isloading, setIsloading] = useState(false);
  const {toast} = useToast()

  const form = useForm<FormDataRegisterMember>({
    resolver: zodResolver(SchemaRegisterMember),
    defaultValues: {
      isBaptized: false,
      isConfirmed: false,
    },
  ...
  const onSubmit = async (values: FormDataRegisterMember) => {
    setIsloading(true);
    const res = await ServiceMember.new("cmij4kmnu0001i5pcgr3q49ft", values);
    if(res.success){
      toast({
        variant:'default',
        description:"Membo registrado com sucesso"
      });
      onCreate(res.result)
    }else{
      toast({
        variant:'destructive',
        description:"Não foi possivel registrar"
      });
      onCreate(null)
    }
    console.log(res);
  ...
  return (
    <DrawerContent className="px-4">
      <DrawerHeader>
        <DrawerTitle>CADASTRAR NOVO MEMBRO</DrawerTitle>
        <DrawerDescription>Adicione od dados</DrawerDescription>
      </DrawerHeader>
      ...
      <Form {...form}>
        ...
        >
            ...
            render={({ field }) => (
              <FormItem>
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="SOLTEIRO">Solteria</SelectItem>
                    <SelectItem value="CASADO">Casado</SelectItem>
                    <SelectItem value="VIUVO">Viuvo</SelectItem>
                    <SelectItem value="VIUVA">Viuva</SelectItem>
                  </SelectContent>
                </Select>
                <FormMessage />
              </FormItem>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex">
                <FormLabel>Foi batizado/a?</FormLabel>
                <Checkbox onChange={(e)=> field.onChange(e)} checked={field.value} />
                <FormMessage />
              </FormItem>
            )}
          />
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex">
                <FormLabel>Batismo Confirmado?</FormLabel>
                <FormControl>
